...
def save_events(events, path):
    ...
    print(f"{C['G']}✓ 已保存 {len(events)} 事件 → {path}{C['W']}")
...
def cmd_watch(args):
    ...
    if hasattr(args, "output") and args.output:
        save_events(matched, args.output)
    return 0
...
def main():
    ...
    p.add_argument("--output", help="保存事件 JSON 到文件 (如 analysis/events.json)")
    ...
    p.add_argument("--output", help="保存匹配事件 JSON 到文件")
    ...
    p.add_argument("--output", help="保存分析结果 JSON 到文件")
