...
static int safe_copy_user_bytes(void *dst, unsigned long user_addr, int maxlen) {
    ...
    __builtin_memset(dst, 0, maxlen);
    ret = compat_copy_from_user(dst, (void *)user_addr, maxlen - 1);
    if (ret != 0) {
        __builtin_memset(dst, 0, maxlen);
        return -1;
    }
...
static unsigned long safe_read_user_ptr(unsigned long user_addr) {
    unsigned long val = 0;
    if (!user_addr) return 0;
    if (compat_copy_from_user(&val, (void *)user_addr, sizeof(val)) != 0)
        return 0;
    return val;
}
...
static void get_current_comm(char *buf, int len) {
    ...
    __builtin_memcpy(buf, task + TASK_COMM_OFFSET, len < TASK_COMM_LEN ? len : TASK_COMM_LEN);
    buf[len - 1] = 0;
    valid = 1;
...
static int write_output_file(const char *path, const char *buf, int len) {
    long fd;
    fd = raw_syscall3(__NR_openat, -100 /* AT_FDCWD */,
                      (unsigned long)path,
...
static void deep_parse_args(struct svc_event *ev) {
    ...
    case __NR_clone:
        ...
        if (g_tier2_enabled && ev->a0) ev->clone_fn = ev->a0;
        break;
    case __NR_clone3:
    ...
    case __NR_execve: {
        ...
        if (ev->a1) {
            unsigned long p = safe_read_user_ptr(ev->a1);
            if (p) {
                safe_copy_user_bytes(arg1, p, sizeof(arg1));
                off += snprintf(ev->desc + off, MAX_DESC_LEN - off, " argv[0]=\"%s\"", arg1);
            }
        }
        break;
    }
...
static int install_hook(int nr) {
    switch (nr) {
#define CASE_HOOK(N) case N: return hook_syscalln(N, 6, before_##N, 0)
...
static long svc_init(const char *args, const char *event, void *reserved) {
    ...
    __builtin_memset(g_events, 0, sizeof(g_events));
    __builtin_memset(g_nr_bitmap, 0, sizeof(g_nr_bitmap));
    g_head = 0; g_tail = 0; g_count = 0;
    g_total = 0; g_seq = 0;
    g_enabled = 0;
    g_uid_filter = -1;
