...
class MainActivity : AppCompatActivity() {
    ...
    private val cPrimary   = Color.parseColor("#2563EB")
    private val cBg        = Color.parseColor("#F8FAFC")
    private val cCard      = Color.parseColor("#FFFFFF")
    private val cCardBg    = Color.parseColor("#FFFFFF")
    private val cCardBorder = Color.parseColor("#E2E8F0")
    private val cText      = Color.parseColor("#1E293B")
    private val cSecondary = Color.parseColor("#64748B")
    private val cGreen     = Color.parseColor("#16A34A")
    private val cRed       = Color.parseColor("#DC2626")
    private val cAccent    = Color.parseColor("#EA580C")
    private val cBtnBg     = Color.parseColor("#2563EB")
    private val cBtnText   = Color.WHITE
    private val cDangerBg  = Color.parseColor("#FEF2F2")
    private val cTabBg     = Color.parseColor("#FFFFFF")
    private val cTabBorder = Color.parseColor("#E2E8F0")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Load app list async to avoid blocking onCreate
        lifecycleScope.launch(Dispatchers.Default) {
            val apps = loadVisibleApps(appSearchQuery)
    ...
    private fun buildDashboardTab(): View {
        ...
        col.addView(makeCard {
            ...
            tvStatus = makeValue("未知"); statusRow.addView(tvStatus)
            statusRow.addView(View(this@MainActivity).apply {
                layoutParams = LinearLayout.LayoutParams(dp(8), 1)
            })
            tvMonState = makeChip("未启动", cCardBorder, cSecondary); statusRow.addView(tvMonState)
            addView(statusRow)

            tvVersion = makeValue("版本: —").apply { setTextColor(cSecondary) }; addView(tvVersion)
            tvUid = makeValue("目标 UID: —").apply { setTextColor(cSecondary) }; addView(tvUid)
            tvEventCount = makeValue("事件数: 0").apply { setTextColor(cSecondary) }; addView(tvEventCount)
        ...
        col.addView(makeCard {
            ...
            etAppSearch = EditText(this@MainActivity).apply {
                ...
                background = roundedBorder(0x00000000, cCardBorder, dp(8))
        ...
        tvDashNrCount = makeValue(""); tvDashNrList = makeValue("")
        col.addView(makeCard {
            addView(makeLabel("🔍 步骤 2: 已选系统调用"))
            addView(tvDashNrCount)
            addView(tvDashNrList.apply {
                maxLines = 4
                ellipsize = TextUtils.TruncateAt.END
        ...
        col.addView(makeCard {
            ...
            btnStartStop = makeBtnPrimary("一键启用监控") { onStartStopClick() }
    ...
    private fun buildFilterTab(): View {
        ...
        col.addView(makeCard {
            ...
            addView(makeBtnDanger("清空已选 NR") { vm.disableAll() })
        })
        ...
        col.addView(makeCard {
            ...
            StatusParser.presets.forEach { preset ->
                addView(makeBtnOutline("${preset.name}: ${preset.description}") {
    ...
    private var lastAllNrQuery = ""
    private var lastAllNrSelected = emptySet<Int>()

    private fun renderAllNrList(query: String) {
        if (!this::llAllNrList.isInitialized) return
        val selected = currentNrList.toHashSet()
        if (query == lastAllNrQuery && selected == lastAllNrSelected) return
        lastAllNrQuery = query
        lastAllNrSelected = selected
        ...
        for (nr in 0..459) {
            ...
            row.addView(Button(this).apply {
                text = "+"; setTextSize(TypedValue.COMPLEX_UNIT_SP, 11f); isAllCaps = false
                setTextColor(cPrimary); background = roundedBorder(0x00000000, cPrimary, dp(4))
                setPadding(dp(6), dp(1), dp(6), dp(1)); stateListAnimator = null
                setOnClickListener { vm.addNr(nr) }
            })
            row.addView(Button(this).apply {
                text = "−"; setTextSize(TypedValue.COMPLEX_UNIT_SP, 11f); isAllCaps = false
                setTextColor(cRed); background = roundedBorder(0x00000000, cRed, dp(4))
                setPadding(dp(6), dp(1), dp(6), dp(1)); stateListAnimator = null
    ...
    private fun buildEventsTab(): View {
        ...
        topBar.addView(Button(this).apply {
            text = "CSV"; setTextSize(TypedValue.COMPLEX_UNIT_SP, 11f); isAllCaps = false
            setTextColor(cPrimary); background = roundedBorder(0x00000000, cPrimary, dp(6))
            setPadding(dp(10), dp(4), dp(10), dp(4)); setOnClickListener { exportCsv() }
            layoutParams = LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT,
                ViewGroup.LayoutParams.WRAP_CONTENT
        ...
        topBar.addView(Button(this).apply {
            text = "JSON"; setTextSize(TypedValue.COMPLEX_UNIT_SP, 11f); isAllCaps = false
            setTextColor(cPrimary); background = roundedBorder(0x00000000, cPrimary, dp(6))
            setPadding(dp(10), dp(4), dp(10), dp(4)); setOnClickListener { exportJson() }
            layoutParams = LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT,
                ViewGroup.LayoutParams.WRAP_CONTENT
        ...
        topBar.addView(Button(this).apply {
            text = "清空"; setTextSize(TypedValue.COMPLEX_UNIT_SP, 11f); isAllCaps = false
            setTextColor(cRed); background = roundedBorder(0x00000000, cRed, dp(6))
            setPadding(dp(10), dp(4), dp(10), dp(4)); setOnClickListener { vm.clearEvents() }
            layoutParams = LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT,
                ViewGroup.LayoutParams.WRAP_CONTENT
            ).apply { marginStart = dp(4) }
        ...
        // Row 2: TID filter + action buttons
        ...
        fun addSmallBtn(label: String, color: Int, onClick: () -> Unit) {
            row2.addView(Button(this).apply {
                text = label; setTextSize(TypedValue.COMPLEX_UNIT_SP, 11f); isAllCaps = false
                setTextColor(color); background = roundedBorder(0x00000000, color, dp(6))
                setPadding(dp(8), dp(4), dp(8), dp(4)); stateListAnimator = null
        ...
        addSmallBtn("清除", cSecondary) { etEventSearch.setText(""); etTid.setText("") }
        addSmallBtn("历史", cPrimary) { shareHistory() }
        addSmallBtn("清历史", cRed) { clearHistory() }

        searchBar.addView(row2)
    ...
    private fun observeViewModel() {
        ...
        vm.monitoring.observe(this) { mon ->
            if (mon) {
                ...
                tvMonState.background = roundedBorder(Color.parseColor("#DCFCE7"), cGreen, dp(4))
                btnStartStop.text = "停止监控"
                btnStartStop.setTextColor(cRed)
                btnStartStop.background = roundedBorder(cDangerBg, cRed, dp(8))
            } else {
                tvMonState.text = "○ 未启动"
                tvMonState.setTextColor(cSecondary)
                tvMonState.background = roundedBorder(0x00000000, cCardBorder, dp(4))
                btnStartStop.text = "一键启用监控"
                btnStartStop.setTextColor(cBtnText)
                btnStartStop.background = roundedBg(cBtnBg, dp(8))
            }
        }
    ...
    private fun updateEventList(events: List<StatusParser.SvcEvent>) {
        ...
        display.forEach { evt ->
            val card = LinearLayout(this).apply {
                orientation = LinearLayout.VERTICAL
                background = roundedBorder(cCardBg, cCardBorder, dp(10))
                setPadding(dp(12), dp(10), dp(12), dp(10))
                layoutParams = LinearLayout.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT,
                    ViewGroup.LayoutParams.WRAP_CONTENT
            ...
            // Row 1: name(NR) · seq · category badge
            val row1 = LinearLayout(this).apply {
                orientation = LinearLayout.HORIZONTAL
                gravity = Gravity.CENTER_VERTICAL
            }
            row1.addView(TextView(this).apply {
                text = "${evt.name}(${evt.nr})"
                setTextSize(TypedValue.COMPLEX_UNIT_SP, 14f)
                setTextColor(cPrimary)
                typeface = Typeface.DEFAULT_BOLD
                layoutParams = LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f)
            ...
            row1.addView(TextView(this).apply {
                ...
                setTextSize(TypedValue.COMPLEX_UNIT_SP, 10f)
                setTextColor(cAccent)
                background = roundedBorder(Color.parseColor("#FFF7ED"), cAccent, dp(4))
            ...
            card.addView(TextView(this).apply {
                text = "tgid=${evt.tgid}  pid=${evt.pid}  uid=${evt.uid}  ${evt.comm}"
                setTextSize(TypedValue.COMPLEX_UNIT_SP, 11f)
                setTextColor(cSecondary)
                setPadding(0, dp(4), 0, 0)
            ...
            // Row 3: description (deep-parsed args)
    ...
    private fun roundedBg(color: Int, radius: Int = dp(12)): android.graphics.drawable.GradientDrawable {
    ...
    private fun roundedBorder(bg: Int, border: Int, radius: Int = dp(12)): android.graphics.drawable.GradientDrawable {
        return android.graphics.drawable.GradientDrawable().apply {
            setColor(bg)
            cornerRadius = radius.toFloat()
    ...
    private fun makeCard(block: LinearLayout.() -> Unit): LinearLayout {
        return LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            background = roundedBorder(cCardBg, cCardBorder, dp(12))
    ...
    private fun makeChip(text: String, bgColor: Int, textColor: Int = Color.WHITE): TextView {
        return TextView(this).apply {
            this.text = text
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 11f)
            setTextColor(textColor)
            typeface = Typeface.DEFAULT_BOLD
            background = roundedBg(bgColor, dp(4))
            setPadding(dp(8), dp(3), dp(8), dp(4))
            gravity = Gravity.CENTER
        }
    }

    private fun makeBtnPrimary(text: String, onClick: () -> Unit): Button {
        return Button(this).apply {
            this.text = text
            setTextColor(cBtnText)
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 14f)
            typeface = Typeface.DEFAULT_BOLD
            isAllCaps = false
            background = roundedBg(cBtnBg, dp(8))
            setOnClickListener { onClick() }
            layoutParams = LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT
    ...
    private fun makeBtnOutline(text: String, onClick: () -> Unit): Button {
        return Button(this).apply {
            this.text = text
            setTextColor(cPrimary)
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 13f)
            isAllCaps = false
            background = roundedBorder(0x00000000, cPrimary, dp(8))
            setOnClickListener { onClick() }
            layoutParams = LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT
    ...
    private fun makeBtnDanger(text: String, onClick: () -> Unit): Button {
        return Button(this).apply {
            this.text = text
            setTextColor(cRed)
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 13f)
            isAllCaps = false
            background = roundedBorder(0x00000000, cRed, dp(8))
            setOnClickListener { onClick() }
            layoutParams = LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT
