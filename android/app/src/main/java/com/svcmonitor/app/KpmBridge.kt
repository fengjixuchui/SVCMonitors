...
import com.svcmonitor.app.jni.Supercall
...
object KpmBridge {
    ...
    private fun friendlyShellError(text: String, exitCode: Int): String {
        val line = firstMeaningfulLine(text)
        val low = line.lowercase()
        return when {
            low.contains("not found") || low.contains("no such file") ->
                "KernelPatch/APatch 控制命令不可用: $line"
            low.contains("permission denied") ->
                "没有执行 KernelPatch/APatch 控制命令的权限: $line"
            line.isNotEmpty() ->
                line
            else ->
    ...
    private fun buildCtl0Command(template: String, command: String): String {
        return if (template.contains("{KEY}")) {
            template
                .replace("{KEY}", superKey)
                .replace("{MODULE}", MODULE)
                .replace("{CMD}", command)
        } else {
            "$template $superKey kpm ctl0 $MODULE '$command'"
        }
    }

    private fun tryCtl0(command: String): Pair<Int, String> {
        // First try native JNI supercall (in-process, no su needed)
        if (Supercall.ensureLoaded()) {
            try {
                val out = Supercall.nativeCtl0(superKey, MODULE, command)
                Log.d(TAG, "tryCtl0 JNI OK: ${out.take(200)}")
                return Pair(0, out)
            } catch (e: Exception) {
                Log.w(TAG, "tryCtl0 JNI failed: ${e.message}")
            }
        }
        // Fall back to shell-based ctl0
        val order = buildList {
            ctl0Resolved?.let { add(it) }
            for (p in ctl0Candidates) {
                if (!contains(p)) add(p)
            }
        }
        var last = Pair(-1, "")
        for (template in order) {
            val shellCmd = buildCtl0Command(template, command)
            val result = shellExec(shellCmd)
            val output = result.second
            last = result
            if (looksLikeJson(output)) {
                ctl0Resolved = template
                return result
            }
            val low = output.lowercase()
            val missingCmd = low.contains("not found") || low.contains("no such file")
            val authFail = low.contains("permission denied") || low.contains("invalid key") ||
                           low.contains("authentication failed") || low.contains("unauthorized")
            if (authFail) {
                Log.w(TAG, "tryCtl0 AUTH_FAIL for template=$template, output=${output.take(200)}")
                return Pair(result.first, output)
            }
            if (!missingCmd) {
                ctl0Resolved = template
                return result
     ...
     */
    private val suBinary = "/system/bin/su"

    /**
     * Execute a command via su (absolute path) — needed because app PATH
     * doesn't include /system/bin on modern Android.
     */
    private fun shellExecDirect(cmd: String): Pair<Int, String> {
        return try {
            Log.d(TAG, "shellExecDirect: $cmd")
            val process = Runtime.getRuntime().exec(arrayOf(suBinary, "-c", cmd))
            val stdout = process.inputStream.bufferedReader().readText().trim()
            val stderr = process.errorStream.bufferedReader().readText().trim()
            val exitCode = process.waitFor()
            val combined = if (stdout.isNotEmpty()) stdout else stderr
            Log.d(TAG, "shellExecDirect exit=$exitCode out=${combined.take(200)}")
            Pair(exitCode, combined)
        } catch (e: Exception) {
            Log.e(TAG, "shellExecDirect error", e)
    ...
    private object PersistentSuShell {
        ...
        private fun ensure(): Boolean {
            ...
            try {
                val np = ProcessBuilder(suBinary).redirectErrorStream(true).start()
                proc = np
    ...
    private suspend fun execute(command: String): KpmResult = mutex.withLock {
        withContext(Dispatchers.IO) {
            try {
                val (exitCode, output) = tryCtl0(command)

                if (output.isNotEmpty()) {
                    if (!looksLikeJson(output)) {
                        val errMsg = friendlyShellError(output, exitCode)
                        Log.w(TAG, "execute($command) SHELL_FAIL: $errMsg")
                        return@withContext KpmResult(false, output, errMsg)
                    }
    ...
    suspend fun drain(max: Int = 1024): KpmResult = mutex.withLock {
        withContext(Dispatchers.IO) {
            try {
                val (exitCode, directOutput) = tryCtl0("drain $max")
                ...
                if (output.isNotEmpty()) {
                    if (!looksLikeJson(output)) {
                        val errMsg = friendlyShellError(output, exitCode)
                        Log.w(TAG, "drain($max) SHELL_FAIL: $errMsg")
                        return@withContext KpmResult(false, output, errMsg)
                    }
                    val simple = StatusParser.parseSimple(output)
                    if (simple.ok) {
