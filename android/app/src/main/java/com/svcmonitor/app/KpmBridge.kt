...
object KpmBridge {
    ...
    suspend fun isModuleLoaded(): Boolean {
        val output = ctlMutex.withLock {
            execRoot(KPATCH_PATH, superKey, "kpm", "list")
        }
        return output.contains(MODULE_NAME)
    }

    suspend fun readProcMaps(pid: Int): String {
        return ctlMutex.withLock {
            execRoot("cat", "/proc/$pid/maps")
        }
    }
}
